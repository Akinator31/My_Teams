...
fn get_user_info(line: &str) -> Option<(String, String, bool)> {
    ...
    let status: bool = parts[2].trim_matches('"').parse().ok()?;
...
pub fn info(io_manager: &mut IoManager, args: &str) {
    ...
    match code {
        ...
        Some(240) => {
            print_colored_reply(&reply);
            if let Some((uuid, name, description)) = format_for_channel_and_team(&reply) {
        ...
        Some(242) => {
            print_colored_reply(&reply);
            if let Some((uuid, creator_uuid, title, msg, timestamp)) = format_thread_info(&reply) {

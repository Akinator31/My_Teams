...
impl From<(String, String, String, String, i64)> for ThreadCreatedEvent {
...
impl MyTeamsSave for Thread {
    fn save(&self, save_file: &mut File) {
        match writeln!(
            save_file,
            "THREAD \"{}\" \"{}\" \"{}\" \"{}\" \"{}\" \"{}\" \"{}\"",
            self.team, self.channel, self.uuid, self.author, self.title, self.body, self.timestamp
        ) {
            Ok(_) => {}
            Err(e) => {
...
impl MyTeamsSave for Reply {
    ...
    fn load(args: &[&str]) -> Option<Self> {
        match args {
            [uuid, team, channel, thread, user_uuid, body, timestamp] => Some(Reply {
                uuid: uuid.trim_matches('"').to_string(),
